...
export default function FeaturedCollectionsBackgroundManager() {
  const { adminToken } = useAuth();
  const [items, setItems] = useState<FeaturedCollectionsBackground[]>([]);
  ...
  const loadBackgrounds = async () => {
    ...
    try {
      setIsLoading(true);
      setError('');
      const response = await fetch(buildApiUrl('/api/landing/featured-collections/backgrounds/admin'), {
        headers: getApiHeaders(adminToken),
      });
      const payload = await response.json();
      if (!response.ok || !payload.success) {
        throw new Error(payload.message || 'Failed to load featured collections backgrounds');
      }
      const nextItems = (payload.data || []) as FeaturedCollectionsBackground[];
      setItems(nextItems);
      setDisplayOrder(nextItems.length);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load featured collections backgrounds');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCreate = async () => {
    if (!adminToken) return;
    if (!selectedFile) {
      setError('Please choose a background image');
      return;
    }
    try {
      setIsSaving(true);
      ...
      uploadFormData.append('folder', 'featured-collections-backgrounds');
      ...
      const response = await fetch(buildApiUrl('/api/landing/featured-collections/backgrounds'), {
        method: 'POST',
        headers: getApiHeaders(adminToken),
        body: JSON.stringify({ title, imageUrl: uploadPayload.url, isActive, displayOrder }),
      });
      const payload = await response.json();
      if (!response.ok || !payload.success) {
        throw new Error(payload.message || 'Failed to create featured collections background');
      }
  ...
  const handleActivate = async (item: FeaturedCollectionsBackground) => {
    if (!adminToken) return;
    try {
      setError('');
      const response = await fetch(buildApiUrl(`/api/landing/featured-collections/backgrounds/${item._id}`), {
        method: 'PATCH',
        headers: getApiHeaders(adminToken),
  ...
  const handleDelete = async (item: FeaturedCollectionsBackground) => {
    if (!adminToken) return;
    if (!window.confirm('Delete this featured collections background image?')) return;
    try {
      setError('');
      const response = await fetch(buildApiUrl(`/api/landing/featured-collections/backgrounds/${item._id}`), {
        method: 'DELETE',
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
      <div>
        <h3 className="font-semibold text-gray-900">Featured Collections Background</h3>
        <p className="text-xs text-gray-500 mt-1">Upload a section background image and mark one as active.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="Featured Collections BG" />
        </div>
        <div>
          <label className="block text-xs font-semibold uppercase text-gray-600 mb-1">Display Order</label>
          <input type="number" value={displayOrder} onChange={(e) => setDisplayOrder(Number(e.target.value || 0))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
        </div>
      </div>
      ...
      <button type="button" onClick={handleCreate} disabled={isSaving} className="px-3 py-2 rounded-lg bg-gray-900 text-white text-sm hover:bg-black disabled:opacity-50">
                ...
                <tr key={item._id} className="border-b border-gray-100">
                  ...
                  <td className="px-3 py-2 text-sm">
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => handleActivate(item)} disabled={item.isActive} className="p-1.5 rounded border border-green-300 text-green-700 hover:bg-green-50 disabled:opacity-50" title="Set active">
                        <FiCheck size={14} />
                      </button>
                      <button type="button" onClick={() => handleDelete(item)} className="p-1.5 rounded border border-red-300 text-red-700 hover:bg-red-50" title="Delete">
                        <FiTrash2 size={14} />
                      </button>
                    </div>
                  </td>
