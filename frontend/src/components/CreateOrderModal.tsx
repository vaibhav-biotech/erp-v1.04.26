...
import { FiX, FiPlus, FiTrash2, FiSearch, FiCalendar, FiUser, FiShoppingBag, FiDollarSign } from 'react-icons/fi';
...
export default function CreateOrderModal({ isOpen, onClose, onOrderCreated }: CreateOrderModalProps) {
  ...
  const calculateTotal = () => {
    ...
    const tax = afterDiscount * 0.18; // approx
    const shipping = afterDiscount >= 60 ? 0 : 50;
    return subtotal - customDiscount + tax + shipping; 
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        items: cart.map(item => ({ productId: item._id, name: item.name, quantity: item.quantity, price: item.overridePrice })),
        paymentMethod,
        customDiscount,
